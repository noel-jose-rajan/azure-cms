...
import { getCurrentUserRoles, UserRole } from "../delegationa-admin/service";
...
import {
  ...
  CreateDelegationUserType,
} from "../delegationa-user/schema";
import {
  createUserDelegation,
  createUserDelegationBody,
} from "../delegationa-user/service";
...
const CreateDelegation = () => {
  ...
  const methods = useForm<CreateDelegationUserType>({
  ...
  const { delegate_all, delegate_to_user_id } = watch();
  ...
  const handleCreateDelegation = async () => {
    try {
      ...
      const body: createUserDelegationBody = {
        date_from,
        date_to,
        delegate_all,
        delegate_to: delegate_all ? delegateTo1 : delegateTo2,
      };
      const res = await createUserDelegation(body);

      if (res) {
        showMessage("success", labels.msg.create_success);
        navigate("/user/delegation", { replace: true });
      }
    } catch (e) {
      handleError(e);
    } finally {
