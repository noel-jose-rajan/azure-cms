...
import { getPickListsItems } from "../../../../../../components/ui/search/correspondence/service";
  ...
  return (
    <FormControl fullWidth margin="normal">
      ...
      <Select
        ...
        onChange={(e) =>
          handleValuesChange(fieldName ?? "statusPicklistIDs", e.target.value)
        }
      >
        {Array.isArray(types) &&
          types.map((item) => (
            <MenuItem value={item.pickListId} key={item.pickListId}>
              <Checkbox
                checked={values.multiCriteria?.statusPicklistIDs?.includes(
                  item.pickListId!
                )}
              />{" "}
