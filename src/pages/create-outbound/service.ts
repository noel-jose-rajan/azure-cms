...
export async function getSearchCount({
  ...
  let url = `${
    ENV.API_URL_LEGACY
  }/correspodence/search-count?criteriaValue=${encodeURIComponent(
    query.criteriaValue || ""
  )}&correspondenceTypeCode=${encodeURIComponent(
    query.correspondenceTypeCode || "all"
  )}&receivingEntityType=${query.receivingEntityType || 0}&sendingEntityType=${
    query.sendingEntityType || 0
  }`;
...
export async function search({
  ...
  const url = `${
    ENV.API_URL_LEGACY
  }/correspodence/search?criteriaValue=${encodeURIComponent(
    query.criteriaValue || ""
  )}&correspondenceTypeCode=${encodeURIComponent(
    query.correspondenceTypeCode || "all"
  )}&receivingEntityType=${query.receivingEntityType || 0}&sendingEntityType=${
    query.sendingEntityType || 0
  }&page=${query.page || 0}&size=${query.size || 10}&sort=${encodeURIComponent(
...
export async function getCanUploadNewVersionPermission(
  correspodence_id: string
...
export async function getCanDeletePermission(
  correspodence_id: string
): Promise<boolean | undefined> {
  try {
    const token = Storage.getItem(LOCALSTORAGE.LEGACY_ACCESS_TOKEN);
...
export async function downloadCorrespondence(
  correspodence_id: number | string
) {
  try {
    const token = Storage.getItem(LOCALSTORAGE.LEGACY_ACCESS_TOKEN);
    ...
    const request = await fetch(
      `${ENV.API_URL_LEGACY}/correspodence/download?corrId=${correspodence_id}`,
      {
        method: "GET",
        headers: headers,
      }
...
export const getPickListsItems = async (
  ...
  try {
    ...
    if (response) return (response as any).content as any[];
...
> => {
  try {
    ...
    const response = await apiRequest(
      "GET",
      `/org-unit/get-role-data?roleName=${roleName}&orgUnitCode=${orgUnitCode}`,
      {},
      {
        headers,
      },
    ...
    if (response) return response.group as any as any[];
