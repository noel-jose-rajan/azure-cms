...
import CreateOutboundModal from "./components/create-modal";
import {
  deleteDraft,
...
export default function OutboundPage() {
  ...
  const fetchAllDrafts = async () => {
    try {
      setLoading(true);
      const response = await getDrafts(3, pagination.page);
      ...
      render: (id, record) => (
        <div style={{ display: "flex", gap: 0 }}>
          <Button
            ...
            size="small"
            onClick={() => {
              navigate("/correspondence/outbound/" + record.id);
            }}
  ...
  const downloadCorrespondence = async (draft: DraftType) => {
    if (!draft) return;
    try {
      await downloadCorrespondenceDocument(draft?.id?.toString(), ".docx");
    } catch (e) {
      message.error(
        isEnglish
  ...
  return (
    <FadeInWrapperAnimation enableScaleAnimation={false} animateDuration={0.75}>
      <Col>
        <TitleBar headerText={labels.til.outbound} />
        <Col
          ...
          >
            {/* <ActionMenuItem
              onClick={() => {
                ...
                navigate(selectedRowKeys[0].toString());
              }}
            ...
            />
            <ActionMenuItem
              onClick={downloadCorrespondence}
              isActive={selectedRowKeys.length === 1}
