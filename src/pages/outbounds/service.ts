...
export interface OutboundCorrListType {
  corrId: string;
...
export const getAllInbounds = async () => {
  try {
    ...
    const response = await httpClient.get<OutboundCorrListType[]>(
      `/correspodence/out/draft`,
      {
        headers: {
...
export const downloadCorrespondenceDoc = async (corrId: string) => {
  try {
    ...
    link.download = corrId + "_corrId_outbound_document.tif";
