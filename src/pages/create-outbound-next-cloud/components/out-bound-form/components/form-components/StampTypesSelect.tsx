...
export default function StampTypesSelect({
    handleValuesChange,
}: Props) {

    const { labels, isEnglish } = useLanguage()

    const [types, setTypes] = useState<PickListItemType[] | undefined>([]);
    ...
    useEffect(() => {
        ...
        const fetchPickListItems = async () => {
            ...
            try {
                const data = await getPickListsItems(PICK_LIST_NAME.STAMP_OPTIONS);
                if (isMounted) {
                    setTypes(data || []);
                }
            } catch (err) {
