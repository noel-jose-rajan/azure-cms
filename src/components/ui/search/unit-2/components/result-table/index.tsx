...
interface Props {
    ...
    onRemove?: (removeId?: number | null) => any
...
const ResultTable = ({ onSelect, value: defaultValue, searchProps, multiple = false, onRemove }: Props) => {
    ...
    const handleActionClick = (orgUnit: OrganizationUnit) => {
        if (multiple) {
            // const updatedSelection = [...selectedUnits, orgUnit];
            // setSelectedUnits(() => updatedSelection);
            onSelect && onSelect(orgUnit);
        } else {
            // setSelectedUnits(() => ([orgUnit]));
            onSelect && onSelect(orgUnit);
        }
    };
    ...
    const columns: ColumnsType<OrganizationUnit> = [
        ...
        },
        {
            ...
            render: (_, record) => (
                ...
                (!!!defaultValue?.find(f => f.organizationUnitId === record.organizationUnitId))
                    ?
                    (<Button
                        type="primary"
                        onClick={() => handleActionClick(record)} // Passing the orgUnit
                    >
                        {btn.select}
                    </Button>)
                    : (<Button
                        type="dashed"
                        onClick={() => onRemove && onRemove(record.organizationUnitId!)} // Passing the orgUnit
                    >
                        {btn.reset}
                    </Button>)
