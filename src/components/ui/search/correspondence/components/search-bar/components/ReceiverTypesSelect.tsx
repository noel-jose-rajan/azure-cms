            ...
            <Col span={state === '' ? 0 : 12}>
                ...
                {state === 'internal'
                    ? <OrganizationUnitSearch
                        value={values.multiCriteria?.recievingEntityIDs as any}
                        onSelect={(e) => !Array.isArray(e) && handleValuesChange("multiCriteria.recievingEntityIDs	", [e])}
                        onChange={() => handleValuesChange("multiCriteria.recievingEntityIDs	", [])}
                    />
                    : <ExternalEntitySearch
                        idRequired={true}
                        multiSelect={false}
                        value={values.multiCriteria?.recievingEntityIDs as any}
                        onChange={(e) => handleValuesChange("multiCriteria.recievingEntityIDs	", e)}
